...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const {
    customerName, province, systemSize, panelCount,
    annualProduction, annualSavings, paybackYears,
    irr, co2Avoided, coverage,
  } = req.body;

  if (!systemSize || !annualProduction) {
    return res.status(400).json({ error: 'Missing required proposal data', code: 'VALIDATION_ERROR' });
  }
          ...
          contents: [
            {
              parts: [
                {
                  text: `Write a compelling 3-paragraph executive summary in Spanish for a personalized solar energy proposal.
...
- Address the client by name if provided`,
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      return res.status(504).json({ error: 'Generation timed out', code: 'TIMEOUT' });
    }
